...
class ArgMessage(Converter):
    async def convert(self, ctx: Context, arg: str) -> Message:
        ...
        message = await event.container.getMessage(ctx.bot)
        if message is None:
...
class ArgArchivedEvent(Converter):
    async def convert(self, ctx: Context, arg: str) -> Event:
        ...
        event = EventDatabase.getEventByID(eventID, archived=True)
        if event is None:
            raise BadArgument("No event found with ID {}".format(eventID))
...
class CommandListener(Cog):
    ...
    async def _create_event(self, ctx: Context, date: datetime,
        ...
        container: Container = await Container.send(event, self.bot.eventchannel)
        if not batch:
            await container.updateReactions()
    ...
    async def archive(self, ctx: Context, event: ArgEvent):
        ...
        container = await event.container.getMessage(self.bot)
        if container:
        ...
        # Create new message
        await Container.send(event, self.bot.eventarchivechannel)
    ...
    async def delete(self, ctx: Context, event: ArgEvent):
        ...
        Example: delete 1
        """
        message = event.container.message
